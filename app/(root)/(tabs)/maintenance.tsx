    ...
    <SafeAreaView className="flex-1 bg-accent-100">
      <ScrollView contentContainerClassName="px-5 pb-32" showsVerticalScrollIndicator={false}>
        <View className="mt-4 flex-row items-center justify-between">
          ...
          {tenant && <TouchableOpacity onPress={() => setShowForm(true)} className="size-12 items-center justify-center rounded-2xl bg-primary-300"><Plus size={26} color="#FFFFFF" /></TouchableOpacity>}
        </View>
        ...
        {(tenant ? maintenanceRequests : [
          ...
          return (
            <View key={item.id} className="mb-3 rounded-[24px] bg-white p-4">
              <View className="flex-row items-start"><View className="size-11 items-center justify-center rounded-2xl bg-primary-100">{item.category === "Plumbing" ? <Droplets size={22} color="#2F6BFF" /> : <Zap size={22} color="#2F6BFF" />}</View><View className="ml-3 flex-1"><View className="flex-row items-start justify-between"><Text className="flex-1 pr-2 font-rubik-semibold text-black-300">{item.title}</Text><View className="rounded-full px-2.5 py-1" style={{ backgroundColor: palette.bg }}><Text className="text-[10px] font-rubik-semibold" style={{ color: palette.fg }}>{item.status}</Text></View></View><Text className="mt-1 text-xs font-rubik text-black-100">{item.date} • {item.category} • {item.priority}</Text><Text className="mt-3 text-xs leading-5 font-rubik-medium text-black-200">{item.update}</Text></View></View>
        ...
        {tenant && <TouchableOpacity onPress={() => setShowForm(true)} className="mt-3 h-14 flex-row items-center justify-center rounded-full border border-primary-300 bg-white"><CirclePlus size={20} color="#2F6BFF" /><Text className="ml-2 font-rubik-bold text-primary-300">New maintenance request</Text></TouchableOpacity>}
      </ScrollView>
      ...
      <Modal visible={showForm} transparent animationType="slide" onRequestClose={() => setShowForm(false)}><View className="flex-1 justify-end bg-[#071F4A]/45"><View className="rounded-t-[34px] bg-white px-6 pb-9 pt-5">
        <View className="mb-5 h-1.5 w-12 self-center rounded-full bg-primary-200" /><View className="flex-row items-center justify-between"><Text className="text-2xl font-rubik-bold text-black-300">Report an issue</Text><TouchableOpacity onPress={() => setShowForm(false)} className="size-10 items-center justify-center rounded-full bg-primary-100"><X size={22} color="#17213C" /></TouchableOpacity></View>
        ...
        <TouchableOpacity className="mt-4 h-12 flex-row items-center justify-center rounded-2xl border border-dashed border-primary-300 bg-primary-100"><Camera size={20} color="#2F6BFF" /><Text className="ml-2 font-rubik-semibold text-primary-300">Add photos or video</Text></TouchableOpacity>
