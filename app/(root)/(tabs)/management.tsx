...
type AgentSection = "homes" | "tenants" | "messages" | "payouts" | "agents" | "approvals";
...
export default function Management() {
  ...
  if (owner) {
    ...
    return <SafeAreaView className="flex-1 bg-transparent"><ScrollView contentContainerClassName="px-5 pb-32" showsVerticalScrollIndicator={false}>
      ...
      <ScrollView horizontal showsHorizontalScrollIndicator={false} className="mt-6">{([{ key: "homes", label: "Properties" }, { key: "agents", label: "Agents" }, { key: "approvals", label: "Approvals" }, { key: "payouts", label: "Payouts" }] as const).map((item) => <Chip key={item.key} label={item.label} selected={ownerView === item.key} onPress={() => setActive(item.key)} />)}</ScrollView>
      ...
      {ownerView === "agents" && <><Text className="mt-6 text-lg font-rubik-bold text-black-300">Assigned agents</Text>{[{ name: "Grace Wanjiku", properties: 4, units: 43, collection: 96 }, { name: "John Mwangi", properties: 2, units: 28, collection: 92 }, { name: "Mary Achieng", properties: 2, units: 19, collection: 89 }].map((agent) => <View key={agent.name} className="mt-3 rounded-[22px] bg-white p-4"><View className="flex-row items-center"><View className="size-11 items-center justify-center rounded-full bg-[#102A55]"><Text className="font-rubik-bold text-white">{agent.name[0]}</Text></View><View className="ml-3 flex-1"><Text className="font-rubik-bold text-black-300">{agent.name}</Text><Text className="mt-1 text-xs font-rubik text-black-100">{agent.properties} properties · {agent.units} units</Text></View><Status value="Active" /></View><View className="mt-4 flex-row justify-between border-t border-primary-100 pt-3"><Text className="text-xs font-rubik text-black-100">Collection rate</Text><Text className="font-rubik-bold text-[#159B6C]">{agent.collection}%</Text></View></View>)}</>}
      {ownerView === "approvals" && <><Text className="mt-6 text-lg font-rubik-bold text-black-300">Pending approvals</Text>{[{ title: "Plumbing repair · B-204", amount: 18500, detail: "Quote from Kamau Plumbing" }, { title: "Security invoice · Parkview", amount: 42000, detail: "September service invoice" }].map((approval) => <View key={approval.title} className="mt-3 rounded-[22px] bg-white p-4"><Text className="font-rubik-bold text-black-300">{approval.title}</Text><Text className="mt-1 text-xs font-rubik text-black-100">{approval.detail}</Text><Text className="mt-3 text-lg font-rubik-bold text-black-300">{formatPrice(approval.amount)}</Text><View className="mt-4 flex-row gap-3"><TouchableOpacity className="flex-1 items-center rounded-full border border-primary-200 py-3"><Text className="font-rubik-semibold text-black-200">Request quote</Text></TouchableOpacity><TouchableOpacity className="flex-1 items-center rounded-full bg-[#23C483] py-3"><Text className="font-rubik-bold text-white">Approve</Text></TouchableOpacity></View></View>)}</>}
