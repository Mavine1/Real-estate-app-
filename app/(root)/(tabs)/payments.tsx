    ...
    <SafeAreaView className="flex-1 bg-accent-100">
      <ScrollView contentContainerClassName="px-5 pb-32" showsVerticalScrollIndicator={false}>
        ...
        {tenantPayments.map((item) => (
          <TouchableOpacity key={item.id} className="mb-3 flex-row items-center rounded-[22px] bg-white p-4">
            <View className="size-11 items-center justify-center rounded-2xl bg-[#EAFBF4]"><Ionicons name="checkmark" size={23} color="#159B6C" /></View>
      ...
      <Modal visible={showPay} transparent animationType="slide" onRequestClose={() => setShowPay(false)}>
        <View className="flex-1 justify-end bg-[#071F4A]/45"><View className="rounded-t-[34px] bg-white px-6 pb-9 pt-5">
          ...
          <View className="mt-4 flex-row items-center rounded-2xl bg-[#EAFBF4] p-3"><Ionicons name="shield-checkmark-outline" size={19} color="#159B6C" /><Text className="ml-2 flex-1 text-xs leading-5 font-rubik text-[#117A56]">You will confirm this payment using your M-Pesa PIN on your phone.</Text></View>
          <TouchableOpacity disabled={submitting} onPress={pay} className="mt-5 h-14 flex-row items-center justify-center rounded-full bg-primary-300">{submitting ? <ActivityIndicator color="#FFFFFF" /> : <><Ionicons name="phone-portrait-outline" size={19} color="#FFFFFF" /><Text className="ml-2 font-rubik-bold text-white">Send M-Pesa prompt</Text></>}</TouchableOpacity>
        </View></View>
      </Modal>
    </SafeAreaView>
  );
